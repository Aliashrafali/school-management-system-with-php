...
class PDFWithWatermark extends FPDF {
    ...
    // total amount
    $sql2 = $conn->prepare("SELECT SUM(total_amount) AS total_amount, SUM(paid_amount) AS total_paid, SUM(discount_amount) AS discount_amount FROM tbl_payments WHERE session = ? AND reg_no = ?");
    ...
    if ($result2 && $result2->num_rows > 0) {
        ...
        $pdf = new PDFWithWatermark('L', 'mm', array(250, 150));
        ...
        $pdf->Cell(30, 5, $registration_info ['mobile'], 1, 0, 'C');
        ...
        $pdf->Cell(30, 5, number_format($total_paid, 2), 1, 0, 'C');
        ...
        $pdf->Cell(20, 8, 'Month', 1, 0, 'C');
        $pdf->Cell(20, 8, 'Total', 1, 0, 'C');
        $pdf->Cell(30, 8, 'Adv. Month', 1, 0, 'C');
        $pdf->Cell(20, 8, 'Adv. Amount', 1, 0, 'C');
        $pdf->Cell(20, 8, 'Disc.', 1, 0, 'C');
        $pdf->Cell(20, 8, 'Grand Total', 1, 0, 'C');
        ...
        foreach($data as $row){
            ...
            $pdf->Cell(20, 5, $row['month_year'], 1, 0, 'C');
            $pdf->Cell(20, 5, number_format($row['total_amount'], 2), 1, 0, 'C');
            $pdf->Cell(30, 5, $row['no_of_advance_month'], 1, 0, 'C');
            $pdf->Cell(20, 5, number_format($row['advance_amount'], 2), 1, 0, 'C');
            $pdf->Cell(20, 5, number_format($row['discount_amount'], 2), 1, 0, 'C');
            $pdf->Cell(20, 5, number_format($row['grant_total'], 2), 1, 0, 'C');
